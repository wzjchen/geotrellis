...
""")
  ...
  def getRasterStepOutput(path:String, layerOpt:Option[RasterLayer], reOpt:Option[RasterExtent]) = {
    ...
    Result(reader.readPath(path, layerOpt, reOpt))
  }
    ...
    catalog.getRasterLayerByName(name) match {
      case Some(layer) => {
        ...
        staticCache.get(layer.name) match {
          case Some(bytes) => Result(reader.readCache(bytes, layer, reOpt))
          case None => Result(reader.readPath(path, Some(layer), reOpt))
        }
      }
      case None => StepError("couldn't find '%s'" format name, "")
    }
  }
}
